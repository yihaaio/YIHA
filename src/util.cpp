...
const char * const BITCOIN_CONF_FILENAME = "yihaatech.conf";
const char * const BITCOIN_PID_FILENAME = "yihaatech.pid";
...
#else
    const char* pszModule = "yihaatech";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YihaaTech";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/YihaaTech";
#else
    // Unix
    return pathRet / ".yihaatech";
#endif
#endif
}
